...
const FilterByNameAndRate = () => setMovies(movies.filter(el => el.title.toLowerCase().includes(text.toLowerCase()) && el.rate >= rating))

return (

<div className="App">
<Filter text={text} setText={setText} FilterByNameAndRate={FilterByNameAndRate} resetRate={() =>setRating(0)} searchingStars = {[1, 2, 3, 4, 5].map((index) => {return (<div onClick={() => onSaveRating(index)} onMouseEnter={() => onMouseEnter(index)} onMouseLeave={() => onMouseLeave()}>
    <Rate index={index} rating={rating} hoverRating={hoverRating} /> </div>)})}
/>
<MovieList  movies={movies} text={text} rating={rating} />
